...
theorem isChain_map_of_isChain {S : β → β → Prop} (f : α → β) (H : ∀ a b : α, R a b → S (f a) (f b))
    {l : List α} (p : IsChain R l) : IsChain S (map f l) := by
  induction p
  · simpa using IsChain.nil
  · simpa using IsChain.singleton _
  · simpa using IsChain.cons_cons (H _ _ ‹_›) ‹_›

theorem IsChain.cons_of_head? {R : α → α → Prop} {a : α} {l : List α} (h : IsChain R l)
    (ha : ∀ b, l.head? = some b → R a b) : IsChain R (a :: l) :=
  match l with
  | [] => by simpa using List.IsChain.singleton _
  | b::bs => List.IsChain.cons_cons (by simpa using ha) h
...
@[simp]
theorem ForwardCharPredSearcher.matched_mem_toList {p : Char → Bool} {s : Slice}
    {l r : s.Pos} :
    SearchStep.matched l r ∈ (ToForwardSearcher.toSearcher p s).toList ↔
      ∃ h, r = l.next h ∧ p (l.get h) = true := by
  rw [toList_toSearcher]
  simp [toList_positions, mem_positionsFrom]
  grind

@[simp]
theorem ForwardCharPredSearcher.rejected_mem_toList {p : Char → Bool} {s : Slice}
    {l r : s.Pos} :
    SearchStep.rejected l r ∈ (ToForwardSearcher.toSearcher p s).toList ↔
      ∃ h, r = l.next h ∧ p (l.get h) = false := by
  rw [toList_toSearcher]
  simp [toList_positions, mem_positionsFrom]
  grind
