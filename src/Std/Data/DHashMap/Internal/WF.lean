...
theorem toListModel_fst_partition [BEq α] [Hashable α] [EquivBEq α] [LawfulHashable α] {m : Raw₀ α β}
  ...
  rw [partition_eq_partitionₘ, partitionₘ]
  ...
  | cons hd tl ih =>
    ...
    · intro a ha
      split
      · simp only
        rw [containsKey_of_perm (toListModel_insert hmx)]
        rw [containsKey_insertEntry, Bool.or_eq_false_iff]
    ...
    · split
      · simp only
        ...
        rw [List.append_cons, List.perm_append_right_iff]
        rw [insertEntry_of_containsKey_eq_false]
