...
theorem all_eq_all_toListModel {p : (a: α) → β a → Bool} {m : Raw α β} :
  ...
  rw [forIn_eq_forIn_toListModel, forIn_eq_forIn']
  induction toListModel m.buckets with
...
theorem Const.toListModel_beq {β : Type v} [BEq α] [PartialEquivBEq α] [Hashable α] [LawfulHashable α] [BEq β] {m₁ m₂ : Raw₀ α (fun _ => β)}  (h₁ : Raw.WFImp m₁.1) (h₂ : Raw.WFImp m₂.1) :
  ...
  simp [beq, Const.beqModel, Raw.size_eq_length h₁, Raw.size_eq_length h₂, Raw.all_eq_all_toListModel,
    get?_eq_getValue? h₂]
